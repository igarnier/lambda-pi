...
and checkable_term =
  | Inf of inferrable_term
  | Lam of checkable_term
  | Bracket of checkable_term
  | Escape of checkable_term
  | Eval of checkable_term

type value =
  | VNeutral of neutral
  | VLam of (value -> value)
  | VInt of int
  | VCode of checkable_term
...
let bracket b = Bracket b

let escape b = Escape b
...
and pp_checkable fragile fmtr ct =
  ...
  | Bracket body -> Fmt.pf fmtr "[%a]" (pp_checkable false) body
  | Escape body -> Fmt.pf fmtr "${%a}" (pp_checkable false) body
  | Eval body -> Fmt.pf fmtr "!{%a}" (pp_checkable false) body
...
let rec typ_eq ty1 ty2 =
  ...
  | (TCode t1, TCode t2) -> typ_eq t1 t2
  | _ -> false
...
let rec subst_checkable :
 ...
 fun i name ct ->
  ...
  | Bracket ct -> Bracket (subst_checkable i name ct)
  | Escape ct -> Escape (subst_checkable i name ct)
  | Eval ct -> Eval (subst_checkable i name ct)
